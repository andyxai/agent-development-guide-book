...
```mermaid
graph TD
    Platform[OpenClaw 平台]
    
    Platform --> Session[Sessions<br/>会话管理]
    Platform --> SubAgent[Sub-agents<br/>子代理]
    Platform --> Skill[Skills<br/>技能市场]
    Platform --> Tool[Tools<br/>工具集成]
    Platform --> Memory[Memory<br/>记忆持久化]
    Platform --> Cron[Cron<br/>定时调度]
    
    Session --> User1[用户A会话]
    Session --> User2[用户B会话]
    
    Skill --> SkillDoc[文档处理Skill]
    Skill --> SkillPDF[PDF解析Skill]
    
    SubAgent --> Agent1[研究Agent]
    SubAgent --> Agent2[写作Agent]
    
    style Platform fill:#e3f2fd
    style Skill fill:#c8e6c9
```

**图 6-1**: OpenClaw一体化平台架构。Sessions管理多用户会话，Skills提供可复用能力，Sub-agents支持任务分解。
