...
```mermaid
graph LR
    Spec[编写Spec<br/>Markdown规范] --> Generate[AI生成<br/>代码+测试+文档]
    Generate --> Review[审查代码<br/>质量检查]
    Review -->|通过| Merge[合并代码]
    Review -->|不通过| Refine[修正Spec]
    Refine --> Generate
    
    Spec -.->|版本控制| Git[Git管理]
    Merge -.->|持续验证| CI[CI/CD验证]
...
**图 26-1**: Spec驱动开发工作流。Spec作为真相来源，AI生成代码，人类审查，形成闭环。
