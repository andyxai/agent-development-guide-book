...
graph TD
    ...
    Context --> ShortTerm[短期记忆<br/>最近10轮对话]
    Context --> LongTerm[长期记忆<br/>用户偏好/历史]
    Context --> WorkMem[工作记忆<br/>当前任务状态]
    
    ShortTerm --> Resolve{指代消解}
    LongTerm --> Resolve
    WorkMem --> Resolve
    
    Resolve -->|他/这个/刚才| Entity[解析实体]
    Entity --> BuildContext[构建上下文]
    
    BuildContext --> Check{窗口超限?}
    Check -->|是| Compress[压缩历史<br/>摘要关键信息]
    Check -->|否| LLM[调用LLM]
    
    Compress --> LLM
    LLM --> Response[生成回复]
    Response --> Update[更新记忆]
    Update --> ShortTerm
    Update --> LongTerm
...
**图 9-1**: 对话型Agent上下文管理流程。三层记忆协同工作，处理指代消解和窗口限制。
...
### 9.4.1 上下文窗口管理（2026年更新）

**窗口限制**：主流模型上下文窗口从 8K 到 200K 不等，但成本随窗口增大而增加。

> **重要发现（2026年）**：上下文质量比长度更重要！Anthropic研究表明超过100K token的上下文会降低推理质量。

**上下文失败的四种模式**（2026年新发现）：

| 失败模式 | 说明 | 解决方案 |
|---------|------|----------|
| **上下文投毒** | 错误信念进入上下文后被强化 | 定期验证上下文事实性 |
| **上下文干扰** | 超过100K token后模型开始重复历史行为而非推理 | 使用RAG检索相关片段而非全量注入 |
| **上下文混淆** | 无关信息影响模型响应 | 预过滤+重排序（re-ranking） |
| **上下文冲突** | 上下文中矛盾部分导致性能下降39% | 冲突检测与消解机制 |
...
3. **三阶段上下文构建**（2026年最佳实践）：预过滤 → 评分排序 → 最小上下文构建

**新增：动态工具选择（Tool Loadout）**
- 问题：工具数量>30时模型选择准确率显著下降
- 解决：根据查询动态选择Top-5相关工具
- 效果：提升44% function-calling性能，降低77%执行时间

> **最佳实践**：漫剧长对话用 Token 计数限制 6K，超过后用 LLM 摘要早期内容保留设定信息，平衡连贯性和成本。优先保证上下文质量，而非盲目增大窗口。
