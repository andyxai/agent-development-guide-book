...
```mermaid
graph TD
    Client[客户端/Agent] --> API[RESTful API]
    
    API --> Resource[资源设计<br/>URL+HTTP方法]
    Resource --> Auth{认证方式}
    
    Auth -->|简单| APIKey[API Key]
    Auth -->|授权| OAuth[OAuth 2.0]
    Auth -->|内部| JWT[JWT Token]
    
    API --> FuncCall[Function Calling<br/>LLM工具调用]
    FuncCall --> Tool1[工具1: 搜索]
    FuncCall --> Tool2[工具2: 计算]
    FuncCall --> Tool3[工具3: 数据库]
    
    API --> Webhook[Webhook<br/>事件驱动]
    Webhook --> Event1[事件: 数据更新]
    Webhook --> Event2[事件: 任务完成]
    
    style Client fill:#e3f2fd
    style API fill:#fff9c4
    style FuncCall fill:#c8e6c9
```

**图 22-1**: 工具集成架构。RESTful API提供基础接口，Function Calling实现LLM工具调用，Webhook支持事件驱动。
