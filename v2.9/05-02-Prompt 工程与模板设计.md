...
```mermaid
graph TD
    Task[任务需求] --> System[System Prompt<br/>角色定义]
    Task --> User[User Prompt<br/>具体任务]
    
    System --> Structure[结构化Prompt]
    User --> Structure
    
    Structure --> FewShot[Few-shot示例<br/>提供样例]
    Structure --> CoT[Chain-of-Thought<br/>思维链]
    
    FewShot --> LLM[调用LLM]
    CoT --> LLM
    
    LLM --> Output[生成结果]
    Output --> Evaluate{质量评估}
    Evaluate -->|不满足| Iterate[迭代优化]
    Evaluate -->|满足| Final[最终Prompt模板]
    Iterate --> Structure
    
    style Task fill:#e3f2fd
    style Final fill:#c8e6c9
    style Iterate fill:#fff9c4
```

**图 20-1**: Prompt工程迭代流程。从任务需求到结构化设计，通过Few-shot和CoT优化，迭代验证。
