...
```mermaid
graph TD
    User[用户输入任务] --> GroupChat[GroupChat<br/>对话协调器]
    
    GroupChat --> Agent1[Agent 1<br/>设定检查专家]
    GroupChat --> Agent2[Agent 2<br/>逻辑检查专家]
    GroupChat --> Agent3[Agent 3<br/>文风检查专家]
    
    Agent1 -->|发言| Discussion[对话讨论]
    Agent2 -->|发言| Discussion
    Agent3 -->|发言| Discussion
    
    Discussion --> Check{终止条件?}
    Check -->|未满足| GroupChat
    Check -->|满足| Summary[生成总结]
    
    Summary --> Result[输出结果]
    
    style User fill:#e3f2fd
    style Result fill:#c8e6c9
    style GroupChat fill:#fff9c4
```

**图 5-1**: AutoGen多Agent协作架构。GroupChat协调多个Agent对话，通过终止条件控制对话结束。
...
- **不再新增功能**：不再添加新特性或重大改进
- **社区维护**：现在由**社区管理**（community managed），非微软官方团队维护
- **微软合并**：微软将AutoGen与Semantic Kernel合并为**Microsoft Agent Framework (MAF)**
- **MAF发布**：MAF 1.0于2025年10月正式发布（生产就绪），支持A2A和MCP协议
...
**重要更新：AutoGen v0.4架构变更**（2025年）

AutoGen v0.4是**从地面重写**（from-the-ground-up rewrite），采用异步、事件驱动架构：

**v0.4三层API设计**：
1. **Core API**：事件驱动的actor模型，支持跨语言（Python/.NET）
2. **AgentChat API**：基于Core的高层任务驱动API（替代v0.2）
3. **Extensions API**：第三方扩展机制

**重大Breaking Changes**：
- 从同步改为**全异步**（async/await）
- 模型客户端配置：`config_list` → `ChatCompletionClient`
- Agent调用方式：`send()` → `on_messages()`/`on_messages_stream()`
- 缓存机制：不再默认启用，需显式使用`ChatCompletionCache`

**v0.4多Agent编排推荐模式**：
```python
# v0.4 推荐使用AgentTool模式
tool_agent = AgentTool(agent=assistant)
result = await tool_agent.on_messages(messages)
```

> **说明**：v0.2仍在`0.2`分支维护，但强烈建议升级到v0.4。本章代码示例基于v0.2，v0.4请参考[官方迁移指南](https://microsoft.github.io/autogen/0.4.8/user-guide/agentchat-user-guide/migration-guide.html)。
