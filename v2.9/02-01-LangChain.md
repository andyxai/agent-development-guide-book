...
**版本历史**：
- **0.0.x**：实验阶段，API 不稳定（2022-2023）
- **0.1.x - 0.3.x**：稳定阶段，广泛使用（2023-2025）
- **1.x**：正式稳定版本（2025年10月发布），无 Breaking Changes 直到 2.0
  - 当前最新：1.2.x（2026年4月）
  - 要求 Python 3.10+（已放弃 3.9 支持）
  - AgentExecutor标记为遗留，推荐使用create_agent

**迁移成本**：0.3.x → 1.x 迁移成本较低，主要为AgentExecutor → create_agent。

**依赖锁定最佳实践**：
```
# requirements.txt (LangChain 1.x)
langchain>=1.2.0
